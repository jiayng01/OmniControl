...
def evaluate_control(motion_loaders, file):
    ...
    with torch.no_grad():
        for idx, batch in enumerate(motion_loader):
            ...
            # foot skating error
            if n_joints == 21:
                skate_ratio, skate_vel = calculate_skating_ratio_kit(
                    motions.permute(0, 2, 3, 1)
                )  # [batch_size]
            else:
            ...
            # process hint
            mask_hint = (
                hint.view(hint.shape[0], hint.shape[1], n_joints, 3).sum(
                    dim=-1, keepdim=True
                )
                != 0
            )
            ...
            for motion, h, mask in zip(motions, hint, mask_hint):
                control_error = control_l2(
                    motion.unsqueeze(0).numpy(),
                    h.unsqueeze(0).numpy(),
    ...
    print(
        f"---> [{motion_loader_name}] Control L2 dist: {dist_mean:.4f}",
        file=file,
        flush=True,
    )
    ...
    with open(log_file, "w") as f:
        ...
        for replication in range(replication_times):
            ...
            (
                control_l2_dict,
                skating_ratio_dict,
                trajectory_score_dict,
            ) = evaluate_control(motion_loaders, f)
