...
def main():
    ...
    if is_using_data:
        ...
        if hints is not None:
            collate_args = [
                dict(arg, hint=hint) for arg, hint in zip(collate_args, hints)
            ]
    ...
    for rep_i in range(args.num_repetitions):
        ...
        sample = sample_fn(
            model,
            # (args.batch_size, model.njoints, model.nfeats, n_frames),  # BUG FIX: MDM 904e4f1 - this one caused a mismatch between training and inference
